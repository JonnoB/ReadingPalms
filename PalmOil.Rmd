...
ReflectCorrect <-function(df, Band, meta ){
  ...
  #Sun elevationhas to be converted to radians
...
df2 <- df %>% select(-Class,-B10,-B11)
  ...
  ) %>%  bind_cols %>% bind_cols(., select(df, B10, B11, Class)
                                 )

  #Creates the new variables from the corrected variables
  df <- df %>% mutate(TNDVI = sqrt(((B5-B4)/(B5+B4))+1), 
                                  ForCrop1 = B3/B2,
                                  ForCrop2 = B7/B2,
                                  WatLan1 = B6/B7,
                                  WatLan2= (B4+B5)/(B6*2),
                                  SAVI = ((B5-B4)/(B5+B4+0.5))*(1+0.5),
                                  Tassel_B = 0.3029*B2 +0.2786*B3 + 0.4733*B4+0.508*B6+0.1872*B7,
                                  Tassel_G = -0.2941*B2 -0.243*B3-0.5424*B4+0.7276*B5+0.0713*B6-0.1608*B7,
                                  Tassel_W = 0.1511*B2+0.1973*B3+0.3283*B4+0.3407*B5-0.7117*B6-0.4559*B7,
                                  EVI = 2.5*(B5-B4)/(B5+6*B4-7.5*B2+1))
}


df <- grep("^B",names(df2) %>% map_df(df2,., ReflectCorrect(df2, ., meta) ))
